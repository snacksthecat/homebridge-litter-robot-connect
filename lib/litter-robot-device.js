...
class LitterRobotDevice {
    ...
    setDetails({unitStatus, nightLightActive, cycleCount, cycleCapacity, cyclesAfterDrawerFull}) {
        ...
        // Force refresh on occupancy sensor

        let occupancyAccessory = this.platform.accessories.find(accessory => {
            return accessory.context.deviceId === this.id && accessory.context.type == 'Occupancy';
        })
        let occupancyService = occupancyAccessory.getService(this.platform.Service.OccupancySensor);
        this.setOccupancyService(occupancyService);
    }
