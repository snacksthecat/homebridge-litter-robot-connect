...
class LitterRobotDevice {
    ...
    updateOccupancyService(service) {
        const platform = this.platform;
        this.occupancyService = service;

        let isOccupied = this.getOccupancy();
        this.occupancyService.setCharacteristic(platform.Characteristic.OccupancyDetected).updateValue(isOccupied);
    }
}
